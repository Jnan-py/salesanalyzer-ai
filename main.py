...
from prompts import get_extraction_model, get_analysis_model, get_recom_model

analysis_sys_ins = get_analysis_model()
extraction_sys_ins = get_extraction_model()
recom_sys_ins = get_recom_model()
...
extraction_model = genai.GenerativeModel(
    ...
    system_instruction= extraction_sys_ins
        
)

analysis_model = genai.GenerativeModel(
    model_name="gemini-2.0-flash",
    system_instruction= analysis_sys_ins
)

recommendation_model = genai.GenerativeModel(
    model_name="gemini-2.0-flash",
    system_instruction= recom_sys_ins
)
...
def main():
    ...
    if st.session_state.get("uploaded_filename"):
        st.success(f"Current file being analysed: {st.session_state.uploaded_filename}")
            ...
            for entry in history_entries:
                ...
                with st.expander(exp_title):
                    if history_tab == "Data Visualizer":                                                
                        st.markdown("**Plot Code:** Paste the code in 'Code to Visuals tab' to see the plot")
                        st.code(entry[2])
                    else:
                        st.write(entry[2])
        else:
    ...
    tabs = st.tabs(["Analyzer", "Data Visualizer", "Chatbot Assistant", "Development Recommendations", "Uploaded Files", "Code to Visuals"])

    with tabs[5]:
        st.header("Code to Visuals")
        st.info("Paste the code from the Data Visualizer History tab to generate the plot here.")
        code = st.text_area("Paste your code here", height=500)
        if st.button("Generate Plot"):
            with st.spinner("Generating plot..."):
                run_plotly_code(code)
